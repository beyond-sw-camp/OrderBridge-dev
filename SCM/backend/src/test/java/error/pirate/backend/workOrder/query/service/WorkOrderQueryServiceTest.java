...
class WorkOrderQueryServiceTest {
    ...
    void readWorkOrderList() {
        ...
        LocalDateTime startDate = LocalDateTime.of(2024, 3, 1, 0, 0); // 테스트용 날짜 범위 시작
        LocalDateTime endDate = LocalDateTime.of(2024, 12, 31, 23, 59); // 테스트용 날짜 범위 끝
        int page = 1;
        int size = 5;

        // When : 메소드 실행
        WorkOrderListResponse response = workOrderService.readWorkOrderList(
                warehouseName, workOrderStatus, startDate, endDate, page, size
        );
        ...
        assertEquals(page, response.getCurrentPage(), "현재 페이지 확인");
    }
}