...
import productionReceiving from "@/router/production-receiving.js";
...
const createProductionReceiving = async () => {
  ...
  const productionReceivingList = [];

  for (const workOrder of checkWorkOrderList.value) {
    console.log(workOrder);
    productionReceiving = {
      itemSeq: workOrder.itemSeq,
      productionReceivingItemQuantity: workOrder.workOrderIndicatedQuantity,
      productionReceivingUnitPrice: '',
      productionReceivingItemNote:''
    }
  }
  try {
    ...
    <div class="col-6 d-flex flex-column">
      ...
      <b-form-group label-cols="4" label-cols-lg="2" label-size="default" label="작업지시서" label-for="workOrder">
        <b-input-group class="w-75" size="sm">
          <b-form-input type="text" id="workOrder" placeholder="작업 지시서를 선택해 주세요." v-model="workOrderName" readonly>
          </b-form-input>
          <b-input-group-text>
    ...
    <div class="col-md-10 d-flex flex-column">
      ...
      <div v-for="workOrder in checkWorkOrderList" :key="workOrder.workOrderSeq" class="mx-5 my-3">
        <div class="d-flex flex-row border border-secondary rounded p-3 position-relative">
          ...
          <!-- x 버튼 -->
          <b-button @click="checkWorkOrder(workOrder.workOrderSeq)" variant="light" size="sm" class="button position-absolute" style="top: 10px; right: 10px;">x</b-button>
        </div>
      </div>
      <!-- 확인 버튼 -->
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="modal-footer d-flex justify-content-end border-0">
            <b-button data-bs-dismiss="modal" variant="light" size="sm" class="button ms-2" @click="writeWorkOrder">확인</b-button>
          </div>
          <div style="max-height: 500px; overflow-y: scroll" class="d-flex row justify-content-center align-items-center">
            ...
            <template v-if="workOrderList.length > 0">
              <div v-for="workOrder in workOrderList" :key="workOrder.workOrderSeq" class="list-line row">
                <!-- 체크박스 -->
                <div class="list-body col-1">
                  <input type="checkbox" v-model="checkedItemList[workOrder.workOrderSeq]" @click="checkWorkOrder(workOrder.workOrderSeq)"/>
                </div>
